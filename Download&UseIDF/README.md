# MyLibrary

## Hướng dẫn cài đặt và sử dụng ESP-IDF

### Tải xuống file ESP-IDF sau đây:

1. esp-idf-tools-setup-offline-5.4.3

2. esp-idf-tools-setup-offline-5.5.3

**Note:** 

- Chú ý yêu cầu version idf. Ví dụ với folder xiaozhi-esp32-2.2.3, code trong đây yêu cầu bản idf 5.5 đổ lên mới sử dụng được.

- Để sử dụng cho xiaozhi-esp32-2-2-3 thì tải file ESP-IDF esp-idf-tools-setup-offline-5.5.3

### Tải xuống như bình thường cho tới khi xuất hiện nút **"Finish"**

![Chọn OK](<img/down b1.jpg>) Chọn OK

![B2](<img/down b2.jpg>) Chọn "I accept the agreement" rồi Next

![B3](<img/down b3.jpg>) Chọn Next

![B4](<img/down b4.jpg>) Chọn Next

![B5](<img/down b5.jpg>) Chọn Install rồi đợi cài đặt ESP-IDF, thời gian đợi tầm 10-15 phút

![B6](<img/down b6.jpg>) Chọn Finish

![alt text](IMG/downB7.jpg)